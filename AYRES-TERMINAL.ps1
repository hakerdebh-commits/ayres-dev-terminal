# AYRES DEV TERMINAL 4.3.0 - launcher PowerShell, ASCII-only.
$ErrorActionPreference = 'Stop'
$Host.UI.RawUI.WindowTitle = 'AYRES DEV TERMINAL 4.3.0'
...
$externalIpTest = Join-Path $PSScriptRoot 'Teste-IP-Externo.ps1'
...
function Header {
  ...
  Write-Host '  | ( A | Y | R | E | S | - | 4 | . | 3 | . | 0 )        |' -ForegroundColor Cyan
  Write-Host '  |  \_/ \_/ \_/ \_/ \_/ \_/ \_/ \_/ \_/ \_/ \_/ \_/      |' -ForegroundColor Green
  Write-Host '  |                                                        |' -ForegroundColor DarkGreen
...
do {
  Header
  Write-Host '[1] Entrar no painel completo AYRES DEV 4.3.0' -ForegroundColor Green
  ...
  Write-Host '[9] Testar somente IP publico (detecta porta automaticamente)' -ForegroundColor Cyan
  ...
  switch($choice){
    ...
    '9' {
      & powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -File $externalIpTest
    }
    '0' { break }
